...
Use `audio_story_architect` only for complex, long-form, episodic, multi-threaded, investigation, special-mechanism, or high-memory-load work. It is optional for straightforward stories and never writes prose.
...
3. Use `audio_story_architect` only when structural complexity justifies it.
...
7. Route bounded findings to the main writer or `audio_story_scene_doctor`; use literary texture only for causally sound but generic passages. After any change, rerun full-draft development.
...
12. Invoke `audio_story_completion_gate` and run the protocol-v2 validator.
