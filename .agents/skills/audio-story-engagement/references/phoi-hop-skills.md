...
### `audio_story_architect` — optional structural handoff
...
### `audio_story_scene_doctor` — bounded repair writer
...
### `audio_story_performance_analyst` — optional post-publication analyst
...
6. Invoke `audio_story_trend_researcher` only after the opted-in platform and format are both set. Pass `RESEARCH_MODE: story_intake`, the exact selection, and relevant story context; use its report as constraints, not a plot.
