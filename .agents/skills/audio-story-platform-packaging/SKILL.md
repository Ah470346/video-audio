...
Trend sounds, slang, or meme language require current research and must fit the narrator or packaging context. Invoke `audio_story_trend_researcher` when current opportunity evidence is requested.
...
Every published variant needs a stable `variant_id` so `audio_story_performance_analyst` can connect analytics to the exact packaging decision.
