...
`story-to-audio` must not repair or rewrite the manuscript. Without an explicit user bypass, return the exact gate blockers and route the file back through `audio-story-engagement` -> `audio_story_developmental_editor` -> `audio-story-final-polish` -> post-polish development/clarity -> `audio_story_completion_gate`.
