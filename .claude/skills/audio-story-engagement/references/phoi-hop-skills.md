...
### `audio-story-developmental-editor` — mandatory read-only release gate
...
3. Run no subagent by default. Use `audio-story-clarity-check` only in targeted mode for a real local listening risk or at the user's request.
