...
## Clarity-Check Preflight And Authority

Before Pass 1, confirm that the current full manuscript has received the mandatory `audio-story-clarity-check` pass after all drafting and literary-texture changes. If not, run it now (Claude Code: `.claude/agents/audio-story-clarity-check.md`; Codex: `.codex/agents/audio-story-clarity-check.toml`).

Treat its output as advisory sentence-level evidence. Validate every finding against the complete manuscript, genre promise, approved intent, character voice, deliberate withholding, and active trend/texture choices. Repair every **confirmed** clarity defect; reject false positives instead of flattening direct emotional naming, subtext, fair clue ambiguity, character-owned metaphor, slang/wordplay, or an unrevealed answer. The subagent is read-only and never outranks this skill or the shared coordination priority; final polish owns all manuscript edits.
...
If Passes 1-7 materially rewrote or added depth-reaching, reflective, reveal, dialogue, or transition sentences after the preflight, rerun the clarity-check subagent on the changed passages or the full current manuscript. Context-validate its findings, repair only confirmed defects, and then run the tests below. This is a diagnostic sub-pass inside final polish; it does not become a later content editor or displace final-polish authority.
