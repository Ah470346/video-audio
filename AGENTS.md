...
6. Then invoke custom agent `audio_story_trend_researcher` from `.codex/agents/audio_story_trend_researcher.toml` with the selected platform and format. Pass its `TREND_RESEARCH_REPORT` to `audio-story-engagement` as research boundaries, never as a plot.
...
Use custom agent `audio_story_receipt_validator` from `.codex/agents/audio_story_receipt_validator.toml` when a sidecar needs a fast, deterministic receipt/hash preflight before the completion gate.
