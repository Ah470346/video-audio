...
- Unless the user already chose a voice, first ask which clone voice to use: `adam`, `ngoc huyen`, or `thuy nguyen`.
- After the user chooses, prepare and push the Kaggle job with `tools/prepare_kaggle_voxcpm_job.py --voice adam`, `--voice ngoc_huyen`, or `--voice thuy_nguyen`, using a job dir in `/Users/truongdv/Documents/projects/video-audio/kaggle_jobs` named `<ten-truyen>_<timestamp>` or `<ten-truyen>_<word-limit>_words_<timestamp>` when rendering a word-limited preview, then stop the chat. Do not wait/poll for Kaggle unless the user explicitly asks.
